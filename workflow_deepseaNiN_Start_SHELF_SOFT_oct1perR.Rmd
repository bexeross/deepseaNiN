---
title: "MAREANO - NiN - SHELF - SOFT october 2022 - 1 per R station"
...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_env_2022-10-20.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_otu_2022-10-20.csv")) %>% as.data.frame
...
otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Ophiopleura_borealis")))]!=0)
...
```{r}
otu_6$spRich<-rowSums(otu_6[,c(1:which(colnames(otu_6)=="Ophiopleura_borealis"))]!=0)
