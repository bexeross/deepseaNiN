---
title: "MAREANO - NiN - SHELF SOFT WEST BARENTS WEST (Lophelia extent) excluding 2006 cruise AND sponge spicule bottom"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__mds_r6_1000rep.rds"))
...
env_orig <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SH_WB_W_LOPH_soft_env_2022-10-28.csv")) %>% as.data.frame
otu_orig <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SH_WB_W_LOPH_soft_otu_2022-10-28.csv")) %>% as.data.frame


table(env_orig$SampID2 == otu_orig$SampID)
...
env_filt4<-filter(env_filt3, (SampID %in% notSpSpicSamp$SampID))
SpSpic_check<-filter(env_filt3, !(SampID %in% notSpSpicSamp$SampID))


### check what removing
plot(env_filt4$X.y, env_filt4$Y, ylim=c(min(env_filt4$Y), max(env_filt4$Y)+100000))
points(SpSpic_check$X.y, SpSpic_check$Y, col='red')



#ensure env and otu datasets have same samples in same order
joindat<-left_join(env_filt4, otu_orig, by ='SampID')
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_ex2006_exSpoSpic__cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__ordisurfs_man_sel_adjusted.png"),
          width = 1000,
          height = 800)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__EnvFit_man_sel_cln_adjusted.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_ex2006_exSpoSpic__gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
