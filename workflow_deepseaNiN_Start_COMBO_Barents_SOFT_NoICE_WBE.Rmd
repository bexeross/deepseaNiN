---
title: "MAREANO - NiN - BARENTS_COMBO_Soft_NOICE >1031"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__mds_r6_1000rep.rds"))
...
env_orig <- read.csv(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS__soft_env_2022-11-18.csv")) %>% as.data.frame
otu_orig <- read.csv(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS__soft_otu_2022-11-18.csv")) %>% as.data.frame
...
## MAKE NO ICE DATASETS
```{r}
joinedDat<- left_join(otu_orig,env_orig, by=c("SampID"))

noice<-subset(joinedDat, BO22_icecoverltmax_ss <0.000001)
plot(noice$X.y.y, noice$Y)

noiceXsamp<-subset(noice, !(SampID %in% c("1188_02",
                                        "1190_00",
                                        "1196_01",
                                        "1199_00",
                                        "1201_02",
...
plot(noiceXsamp$X.y.y, noiceXsamp$Y)
noiceXsamp_otu_sort<-noiceXsamp %>% select(c(2:106)) 
noiceXsamp_env_sort<-noiceXsamp %>% select(c(2,109:244))  




# ice<-subset(joinedDat, BO22_icecoverltmax_ss >=0.000001)
# plot(ice$X.y.y,ice$Y)
# otu_sort<-ice %>% select(c(2:106)) 
# env_sort<-ice %>% select(c(2,109:244))  
```


## LOAD WBE dataset
```{r}
env_wbe <- read.csv(file.path(dataPath,"inputs/SH_WB_E_PARAPRIM_soft_env_2022-10-28.csv")) %>% as.data.frame
otu_wbe <- read.csv(file.path(dataPath,"inputs/SH_WB_E_PARAPRIM_soft_otu_2022-10-28.csv")) %>% as.data.frame
table(env_wbe$SampID2 == otu_wbe$SampID)
...
env_wbe$X.y.y<-env_wbe$X.y

env_wbe<-env_wbe %>% select (-c(X.y))

#wbe<- left_join(otu_wbe,env_wbe, by=c("SampID"))
```



## JOIN noiceXsamp and WBE datasets

```{r}
otus<-bind_rows(noiceXsamp_otu_sort, otu_wbe) 
envs<-bind_rows(noiceXsamp_env_sort, env_wbe)

joinCombo<-left_join(otus,envs, by=c("SampID"))

otu_sort<-joinCombo %>% select(c(1:which(colnames(joinCombo)=="VL")-1)) %>%
  select(c("SampID"), sort(colnames(.))) %>%
  select_if(~ !any(is.na(.))) # we cannot keep any taxa which are NA in the other area
env_sort<-joinCombo %>% 
  select(c("SampID",c(which(colnames(joinCombo)=="VL"):which(colnames(joinCombo)=="swDensRob_avs")))) 
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_COMBO_Soft_NOICE__cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_ordisurfs_top_corr.png"),
          width = 800,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_ordisurfs_man_sel_domean.png"),
          width = 1000,
          height = 1000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
c25 <- c(
  ...
  "#FDBF6F", # lt orange
  "gray70", "khaki2",
  "maroon", "orchid1", "BARENTS_COMBO_Soft_NOICEpink1", "blue1", "steelblue4",
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/BARENTS_COMBO_Soft_NOICE_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
...
ggplotly(p_gmo)
```
#ID samples with gnmds >0.5 for trial removal next time

```{r}
Xsamp<-env_vis %>% filter(gnmds1 >0.5) %>% select(SampID)
```
