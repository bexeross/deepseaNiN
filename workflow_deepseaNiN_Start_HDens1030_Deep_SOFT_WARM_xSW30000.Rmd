---
title: "MAREANO - NiN - hiDens >1031, SOFT, WARM, excluding SW(X<30 000m, UTM 33N)"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_mds_r6_1000rep.rds"))
...
env_sort <- read.csv(file.path(dataPath,"inputs/Hdens1030_SOFT_env_2022-11-04.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/Hdens1030_SOFT_otu_2022-11-04.csv")) %>% as.data.frame
...
#Remove samples not at Atl. margin and >2.5*C

```{r}
joinData <- left_join(otu_sort, env_sort, by = "SampID")
#remove data above east of X = 600000
joinData <- subset(joinData, (x_coord>30000)&(x_coord<600000)&(Tmean_Robinson>2.5))


otu_sort<-joinData %>% select(c(2:which(colnames(joinData)=="Umbellula_encrinus")))
env_sort<-joinData %>% select(c(2,(which(colnames(joinData)=="Umbellula_encrinus")+1):which(colnames(joinData)=="sand")))
```
...
otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Umbellula_encrinus")))]!=0)
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```{r}
otu_6$spRich<-rowSums(otu_6[,c(1:which(colnames(otu_6)=="Umbellula_encrinus"))]!=0)
...
env_cont<-env%>% select(-c(landscape,sedclass,gmorph, MLDmean_bathy, MLDmax_bathy, MLDmin_bathy, #categorical
                           ...
                           SampID,
                           sw_spd_max, sw_spd_mean)) #partial coverage
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/DEEP/Hdens1030_SOFT_WARM_xSW___cor-table_r6.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___ordisurfs_top_corr.png"),
          width = 800,
          height = 1000)

```

**MARGARET STOPPED HERE**
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___ordisurfs_man_sel_domean.png"),
          width = 1000,
          height = 1000)
...
colnames(env_os_m_envfit)<-c("T", #top corr
                  ...
                  "X", #comparison to Y
                  "Y", #top corr
                  "spd_sd", #top corr ax2 (blended model)
                  "CSpdsdR", #comparison to top corr ax2 (blended model)
                  "CSpdmeanR", #comparison to top corr ax2 (blended model)
                  "mud", #highest sed var ax1 + corr
                  "sand",
                  "gravel",#highest sed var ax1 - corr
                  "SiMax", #just under top corr ax1
                  "bathy" #intuitive for comparisons
                 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
c25 <- c(
  ...
  "#FDBF6F", # lt orange
  "gray70", "khaki2",
  "maroon", "orchid1", "hiDenspink1", "blue1", "steelblue4",
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/Hdens1030_SOFT_WARM_xSW___gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
...
#map gnmds1
```{r}
xy_t<- ggplot(data = env_vis,
                      aes(x = X.y.y,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = gnmds1),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_vis$gnmds1), max(env_vis$gnmds1)), #match variable acting as colour
                        colors=c('green','yellow','red'))+
  ggtitle("Geography (X v Y) - coloured by mean gnmds1") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens1030_SOFT_WARM_xSW__X_v_Y_gnmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
xy_t<- ggplot(data = env_vis,
  ...
  geom_point(aes(colour = gnmds2),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_vis$gnmds2), max(env_vis$gnmds2)), #match variable acting as colour
                        colors=c('green','yellow','red'))+
  ggtitle("Geography (X v Y) - coloured by mean gnmds2") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens1030_SOFT_WARM_xSW__X_v_Y_gnmds2.png"), #amend
       device = "png",
       dpi=300 )
```