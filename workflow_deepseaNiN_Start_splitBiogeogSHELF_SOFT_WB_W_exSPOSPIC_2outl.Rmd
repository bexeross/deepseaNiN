...
# add cruise number
env_orig_crno<-env_orig %>% select(c('SampID', 'cruise_no'))
env_sub_meta1<-left_join(env_sub_meta1,env_orig_crno,by="SampID")
```
...
xy_t<- ggplot(data = env_sub_meta,
                      aes(x = X,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = mud),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_sub_meta$mud), max(env_sub_meta$mud)), #match variable acting as colour
                        colors=c('yellow','orange','red'))+
  ggtitle("Geography (X v Y) - coloured by mud") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_mud.png"), #amend
       device = "png",
       dpi=300 )
```
...
xy_t<- ggplot(data = env_sub_meta,
                      aes(x = X,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = gravel),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_sub_meta$gravel), max(env_sub_meta$gravel)), #match variable acting as colour
                        colors=c('yellow','orange','red'))+
  ggtitle("Geography (X v Y) - coloured by gravel") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_gravel.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_bathy.png"), #amend
       device = "png",
       dpi=300 )
```
#### X v Y - gnmds1
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_gnmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_gnmds2.png"), #amend
       device = "png",
       dpi=300 )
```
...
cat_xy_ax1 <- ggplot(data = env_sub_meta,
              aes(x = X,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = as.factor(env_sub_meta$cruise_no)), #change at will - categorical vars only
             size = 1) +
 ...
 # scale_colour_brewer(palette = "Spectral") + # ordered colourblind pallette
  ggtitle("Easting vs Northing - coloured by cruise number") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_cruise_no.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC_bathy_v_temp_nmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC_bathy_gnmds_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC_bathy_dca_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__X_v_Y_tmeanR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__gnmds_gmo.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__XvY_ax1cat.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save some outputs
ggexport(comp,
          filename = file.path(dataPath,"outputs/SHELF_SOFT_WB_W_xSPOSPIC__compare.png"), #amend
...
```{r}
#save.image("I:/Scripts/deepseaNiN/Renv_deepseaNiN_SHELF_SOFT_WB_W_xSPOSPIC_.RData") # edit area first
