---
title: "MAREANO - NiN:splitting biogeographic regions_COMBO_Atlantic_WBW"
...
env_sub_meta<- env_sub_meta %>%
  mutate(
    sedmilName = case_when(
            sedmil == "1" ~ "Deposition from suspension",
            sedmil == "2" ~ "Deposition from suspension, local erosion",#Deposition from suspension, local erosion of fine-grained sediments
            sedmil == "7" ~ "No/very slow deposition",
            sedmil == "3" ~ "Deposition from bottom currents",
            sedmil == "4" ~ "Erosion, local deposition",#Erosion, local deposition of sediments in topographic lows
            sedmil == "5" ~ "Erosion"
    ))
...
gnmds_gmo <- ggplot(data = env_sub_meta,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by sedimet origin",
          subtitle = "First run") +
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__gnmds_seddan.png"), #amend
       device = "png",
       dpi=300 )
...
#### Gnmds w erosion deposition
```{r}

gnmds_sm<- ggplot(data = env_sub_meta,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by erosion/deposition",
          subtitle = "First run") +
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__gnmds_sedmil.png"), #amend
       device = "png",
       dpi=300 )
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  ggtitle("Xvs Y - coloured by gnmds r6 ax1") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006_x_vs_y_gnmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
tb_ax1<- ggplot(data = env_sub_meta,
                      aes(x = X, #change at will
                          y = Y)) +#change at will
  theme_classic() +
  geom_point(aes(colour = env_sub_meta$vLophelia_rubble),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_sub_meta$gnmds1),max(env_sub_meta$gnmds1)), #match variable acting as colour
                        colors=c('red','yellow','green'))+
  ggtitle("Xvs Y - coloured by vLophelia_rubble") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__gnmds_biotope.png"), #amend
       device = "png",
       dpi=300 )
...
cat_xy_ax1 <- ggplot(data = env_sub_meta,
              aes(x = X,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = env_sub_meta$BiotopeDesc), #change at will - categorical vars only
             size = 1) +
 scale_colour_manual(values=col_vector) +# non-ordered colourblind pallette
   # scale_colour_brewer(palette = "Spectral") + # ordered colourblind pallette
  ggtitle("Easting vs Northing - coloured by biotope") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__XvY_biotope.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006_bathy_v_temp_nmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006_bathy_gnmds_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006_bathy_dca_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__X_v_Y_tmeanR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__gnmds_gmo.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__XvY_ax1cat.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save some outputs
ggexport(comp,
          filename = file.path(dataPath,"outputs/COMBO_ATL_WBWx2006__compare.png"), #amend
...
```{r}
#save.image("I:/Scripts/deepseaNiN/Renv_deepseaNiN_COMBO_ATL_WBWx2006_.RData") # edit area first
