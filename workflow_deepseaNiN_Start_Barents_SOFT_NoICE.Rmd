---
title: "MAREANO - NiN - BARENTS_Soft_NOICE >1031"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__mds_r6_1000rep.rds"))
...
otu_sort<-noice %>% select(c(2:106)) 
env_sort<-noice %>% select(c(2,109:244))  
...
otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Urasterias_lincki")))]!=0)
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/BARENTS_Soft_NOICE_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/BARENTS_Soft_NOICE_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/BARENTS_Soft_NOICE_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/BARENTS/BARENTS_Soft_NOICE__cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_ordisurfs_top_corr.png"),
          width = 800,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_ordisurfs_man_sel_domean.png"),
          width = 1000,
          height = 1000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
c25 <- c(
  ...
  "#FDBF6F", # lt orange
  "gray70", "khaki2",
  "maroon", "orchid1", "BARENTS_Soft_NOICEpink1", "blue1", "steelblue4",
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/BARENTS_Soft_NOICE_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
