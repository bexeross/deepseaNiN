...
TOC_map<-InsituTOC_locations<-Bio_data %>%
  ggplot(.,  aes(x = lon, y = lat, color = TOC_percent))+geom_point(alpha = 0.7)+theme_classic()+
  scale_color_viridis_c(option = "magma",direction = -1)+
  theme(legend.position = "bottom")+
  ggtitle("with TOC % data")+
  geom_map(data =  ggworldMap, map = ggworldMap, aes(long, lat, map_id = id), fill = "grey75", inherit.aes = FALSE)+
  coord_cartesian(xlim = c(range(Bio_data$lon)), ylim = c(range(Bio_data$lat))) +
  theme(legend.position = "bottom")

Grain_map<-Bio_data %>%
  ggplot(.,  aes(x = lon, y = lat, color = grainsize_percent))+geom_point(alpha = 0.7)+theme_classic()+
  scale_color_viridis_c(option = "magma",direction = -1)+
  theme(legend.position = "bottom")+
  ggtitle("with gransize data")+
  geom_map(data =  ggworldMap, map = ggworldMap, aes(long, lat, map_id = id), fill = "grey75", inherit.aes = FALSE)+
  coord_cartesian(xlim = c(range(Bio_data$lon)), ylim = c(range(Bio_data$lat))) +
  theme(legend.position = "bottom")

TOM_map<-Bio_data %>%
  ggplot(.,  aes(x = lon, y = lat, color = TOM_percent))+geom_point(alpha = 0.7)+theme_classic()+
  scale_color_viridis_c(option = "magma",direction = -1)+
  theme(legend.position = "bottom")+
  ggtitle("with TOM % data")+
  geom_map(data =  ggworldMap, map = ggworldMap, aes(long, lat, map_id = id), fill = "grey75", inherit.aes = FALSE)+
  coord_cartesian(xlim = c(range(Bio_data$lon)), ylim = c(range(Bio_data$lat))) +
  theme(legend.position = "bottom")
...
Subsetting workable Beamtrawl data

```{r}

Dat_gear_long<-Bio_data %>% 
  filter(!(taxa_rank %in% c("Kingdom", "Phylum", "Subphylum", "Superclass", "Class")),
         !(scientific_name %in% c("Actiniaria", 'Cephalaspidea', 'Dendrobranchiata')))
...
# 
# Dat_gear_long %>%
#   group_by(refstation_no, sample_no) %>%
#   count() 
#   group_by(refstation_no, scientific_name) %>%
#   tally()
#   summarise(MeanDen_RStation=mean(nos_100m2, na.rm =TRUE)) %>%
 
   #EG
#Dat_gear_long[Dat_gear_long$refstation_no %in% c("160") & Dat_gear_long$aphia_id == 222,]
...
table(Dat_gear_long$refstation_no, Dat_gear_long$sieve_mesh_size)
...
```{r}
if(SPLIT == "Deep"){
  Dat_gear_long<-Dat_gear_long[Dat_gear_long$SampleID %in% Env_data[Env_data$swDensRob_avs>=1031.8  & !is.na(Env_data$bathy) & (is.na(Env_data$sw_spd_mean) | Env_data$sw_spd_mean<0.3) , 'SamplID'],]}


if(SPLIT == "Barents"){
  Dat_gear_long<-Dat_gear_long[Dat_gear_long$SampleID %in% Env_data[Env_data$swDensRob_avs < 1031.8 & Env_data$BO22_dissoxmean_bdmean > 305 & !is.na(Env_data$bathy) & Env_data$bathy<=100,  'SamplID'],]}
if(SPLIT == "Shelf"){
  Dat_gear_long<-Dat_gear_long[Dat_gear_long$SampleID %in% Env_data[Env_data$swDensRob_avs < 1031.8 & Env_data$BO22_dissoxmean_bdmean <= 305 & !is.na(Env_data$bathy) & Env_data$bathy<=100,  'SamplID'],]
...
```{r , include=FALSE}
# clay_cob<-ggplot(env_corr, aes(percent_clay, cobB))+geom_point()
# clay_grav<-ggplot(env_corr, aes(percent_clay, gravel))+geom_point()
# clay_mud<-ggplot(env_corr, aes(percent_clay, mud))+geom_point()
# clay_rock<-ggplot(env_corr, aes(percent_clay, rock))+geom_point()
# clay_sand<-ggplot(env_corr, aes(percent_clay, sand))+geom_point()
# 
# clay_cob + clay_grav + clay_mud + clay_rock + clay_sand
# 
# silt_cob<-ggplot(env_corr, aes(percent_silt, cobB))+geom_point()
# silt_grav<-ggplot(env_corr, aes(percent_silt, gravel))+geom_point()
# silt_mud<-ggplot(env_corr, aes(percent_silt, mud))+geom_point()
# silt_rock<-ggplot(env_corr, aes(percent_silt, rock))+geom_point()
# silt_sand<-ggplot(env_corr, aes(percent_silt, sand))+geom_point()
# 
# silt_cob + silt_grav + silt_mud + silt_rock + silt_sand
# 
# sand_cob<-ggplot(env_corr, aes(percent_sand , cobB))+geom_point()
# sand_grav<-ggplot(env_corr, aes(percent_sand , gravel))+geom_point()
# sand_mud<-ggplot(env_corr, aes(percent_sand , mud))+geom_point()
# sand_rock<-ggplot(env_corr, aes(percent_sand , rock))+geom_point()
# sand_sand<-ggplot(env_corr, aes(percent_sand , sand))+geom_point()
# 
# sand_cob + sand_grav + sand_mud + sand_rock + sand_sand
# 
# grav_cob<-ggplot(env_corr, aes(percent_gravel, cobB))+geom_point()
# grav_grav<-ggplot(env_corr, aes(percent_gravel, gravel))+geom_point()
# grav_mud<-ggplot(env_corr, aes(percent_gravel, mud))+geom_point()
# grav_rock<-ggplot(env_corr, aes(percent_gravel, rock))+geom_point()
# grav_sand<-ggplot(env_corr, aes(percent_gravel, sand))+geom_point()
# 
# grav_cob + grav_grav + grav_mud + grav_rock + grav_sand
...
for (i in seq(ncol(env_os_m2))) {
  if(sum(is.na(env_os_m2[, i]))/length(env_os_m2[, i])>0.90){
  ordsrfs[[i]]<-NA
  next}
  os.i_m <- gg_ordisurf(ord = ord,
                        env.var = env_os_m2[, i],
...
# `{r} # env_vis<-otu_6 # env_vis$gnmds1 <- otu_6$gnmds1 # env_vis$gnmds2 <- otu_6$gnmds2 #  # env_vis$dca1 <- otu_6$dca1 # env_vis$dca2 <- otu_6$dca2 #`
...
```{r}
p_dca_plotly <- ggplot(data = otu_6,
                     aes(x = dca1,
                         y = dca2)) +
  theme_classic() +
  coord_fixed() +
...
map_samples5<-ggplot(data = env_vis,
  ...
  theme_classic() +
  geom_point(aes(colour = gnmds1)) +scale_color_viridis_c()+
  theme_classic() 
...
```{r}
map_samples2_wden<-ggplot(data = env_vis,
                    aes(x = lon,
                        y = lat)) +
  theme_classic() +
  ggtitle("Grab split (water density)") +
  geom_point(aes(colour = swDensRob_avs > 1030.101)) +
...
map_samples2_depth<-ggplot(data = env_vis,
                    aes(x = lon,
                        y = lat)) +
  theme_classic() +
  ggtitle("Grab split (depth)") +
  geom_point(aes(colour = bathy < -498))+
  scale_colour_manual(values = setNames(c("#ae2012", "#005f73"),c(T, F))) +
  theme_classic() +theme(legend.position = "none", axis.text.y = element_text(angle = 90, vjust = 0.5, hjust=0.5))

map_samples2_video<-ggplot(data = env_vis,
                    aes(x = lon,
                        y = lat)) +
  theme_classic() +
  ggtitle("Video split") +
  geom_point(aes(colour = swDensRob_avs > 1030)) +
  scale_colour_manual(values = setNames(c("#ae2012", "#005f73"),c(T, F))) +
  theme_classic() +theme(legend.position = "none", axis.text.y = element_text(angle = 90, vjust = 0.5, hjust=0.5))

map_samples2_video + map_samples2_wden   + map_samples2_depth

##############################
map_samples2_wden_CS<-ggplot(data = env_vis,
                    aes(x = lon,
                        y = lat)) +
  theme_classic() +
  ggtitle("Grab split (water density)") +
  geom_point(aes(colour = swDensRob_avs > 1030.101)) +
  geom_point(data = env_vis[env_vis$swDensRob_avs > 1030 & env_vis$spd_mean >0.25,], colour = "green") +
  scale_colour_manual(values = setNames(c("#ae2012", "#005f73"),c(T, F))) +
  theme_classic() +theme(legend.position = "none", axis.text.y = element_text(angle = 90, vjust = 0.5, hjust=0.5))
...
map_samples2_video_CS<-ggplot(data = env_vis,
                    aes(x = lon,
                        y = lat)) +
  theme_classic() +
  ggtitle("Video split") +
...
map_samples2_video_CS + map_samples2_wden_CS   + map_samples2_depth_CS

##########
gnmds_samples2_wden<-ggplot(data = env_vis,
                    aes(x = gnmds1,
                        y = gnmds2)) +
  theme_classic() +
  ggtitle("Grab split (water density)") +
...
gnmds_samples2_depth<-ggplot(data = env_vis,
                    aes(x = gnmds1,
                        y = gnmds2)) +
  theme_classic() +
  ggtitle("Grab split (depth)") +
...
gnmds_samples2_video<-ggplot(data = env_vis,
                    aes(x = gnmds1,
                        y = gnmds2)) +
  theme_classic() +
  ggtitle("Video split") +
...
gnmds_samples2_video + gnmds_samples2_wden   + gnmds_samples2_depth

##############################
gnmds_samples2_wden_CS<-ggplot(data = env_vis,
                    aes(x = gnmds1,
                        y = gnmds2)) +
  theme_classic() +
  ggtitle("Grab split (water density)") +
...
gnmds_samples2_video_CS<-ggplot(data = env_vis,
                    aes(x = gnmds1,
                        y = gnmds2)) +
  theme_classic() +
  ggtitle("Video split") +
...
gnmds_samples2_video_CS + gnmds_samples2_wden_CS   + gnmds_samples2_depth_CS
...
Wden_samples<-ggplot(data = env_vis,
                    aes(x =  gnmds1, 
                        y = swDensRob_avs)) +
  geom_hline(yintercept = 1030.101) +
  geom_hline(yintercept = 1030, color = "#f28482") +
  ggtitle("Video vs grab split") +
  geom_point(aes(colour = spd_mean)) +
  scale_color_viridis_c()+
  theme_classic() 

ggplotly(Wden_samples)

bathy_samples<-ggplot(data = env_vis,
                    aes(x =  gnmds1, 
                        y = bathy)) +
  geom_hline(yintercept = -495) +
  ggtitle("Video vs grab split") +
  geom_point(aes(colour = spd_mean)) +
  scale_color_viridis_c()+
  theme_classic() 

ggplotly(bathy_samples)

bathy_samples2<-ggplot(data = env_vis,
                    aes(x =  gnmds1, 
                        y = bathy)) +
  geom_hline(yintercept = -495) +
  ggtitle("Video vs grab split") +
