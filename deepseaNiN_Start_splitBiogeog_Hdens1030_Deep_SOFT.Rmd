---
title: "MAREANO - NiN:splitting biogeographic regions_Deep (Hdens1030) Soft"
...
env_sub_meta1$spRich<-rowSums(otu_6[,-c(1:which(colnames(otu_6)=="Umbellula_encrinus"))]!=0)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_bathy_v_temp_nmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
xy_t<- ggplot(data = env_sub_meta,
  ...
  ggtitle("Geography (X v Y) - coloured by mean bathy") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_sedmil.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Hdens1030_SOFT_biotope.png"), #amend
       device = "png",
       dpi=300 )
