...
sandwave area shelf only


```{r}

env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_env_sort2022-10-14.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_otu_sort2022-10-14.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/sandwave_shelf_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/sandwave_shelf_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/sandwave_shelf_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```
```{r}
ggsave(file.path(dataPath,"outputs/sandwave_shelf_gnmds_SpRichr6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_shelf_cor-table_r6_200rep.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/sandwave_shelf_ordisurfs_top_corr.png"),
          width = 2000,
          height = 2500)
...
### Sel env var (manual)
```{r}
env_os_m <- env[,c("bathy", #top corr
                  "mud", #highest sed var ax1 + corr
                  "gravel",#highest sed var ax1 - corr
                  "sw_spd_mean", #interest for VF LKM
                  "sw_spd_max" #interest for VF LKM
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/sandwave_shelf_ordisurfs_man_sel_domean.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/sandwave_shelf_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
