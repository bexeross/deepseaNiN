...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/BARENTS/LDnoSBhi02_env_sort_no_ice_2022-09-29.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/BARENTS/LDnoSBhi02_otu_sort_no_ice_2022-09-29.csv")) %>% as.data.frame
...
joinedDat1<-subset(joinedDat, bathy <= -99) # subsetting only stations below 100m depth
summary(joinedDat1$bathy)

joinedDat_with_ice<-subset(joinedDat1, BO22_icecoverltmax_ss >= 0.1) # subsetting only stations below 100m depth with ice
joinedDat_no_ice<-subset(joinedDat1, BO22_icecoverltmax_ss <= 0) # subsetting only stations below 100m depth with no ice
summary(joinedDat_no_ice$BO22_icecoverltmax_ss)
summary(joinedDat_with_ice$BO22_icecoverltmax_ss)

env_sort<-joinedDat1 %>% select(c(2:351))
otu_sort<-joinedDat1 %>% select(c(2,352:491))

env_sort_no_ice_no_ice<-joinedDat_no_ice %>% select(c(2:351)) # no ice
otu_sort_no_ice_no_ice<-joinedDat_no_ice %>% select(c(2,352:491))

env_sort_with_ice<-joinedDat_no_ice %>% select(c(2:351)) # with ice
otu_sort_with_ice<-joinedDat_no_ice %>% select(c(2,352:491))
...
env_sort_no_ice <- env_sort_no_ice %>% select(-c("BO22_lightbotmean_bdmean",
...
```{r}
env_sort_no_ice_locations<- ggplot(data = env_sort_no_ice,
                      aes(x = X.y,
                          y = Y)) +
  theme_classic() +
...
```{r}
## Removing NAs
otuCompl <- otu_sort_no_ice[complete.cases(env_sort_no_ice[, -c(1:which(colnames(env_sort_no_ice)=="bathy")-1)]), ] # make sure we have the columms 
envCompl <- env_sort_no_ice[complete.cases(env_sort_no_ice[, -c(1:which(colnames(env_sort_no_ice)=="bathy")-1)]), ]
...
```{r}
envCompl_ccrem<-env_sort_no_ice%>%filter(!SampID%in%envCompl$SampID)
...
env<-envSel[,-c(1: which(colnames(envSel)=="bathy")-1)]
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/Barents_below100_no_ice_geodist_r6.rds")))
```
...
```{r}
saveRDS(Xmds_r6,
        file = file.path(dataPath,"inputs/Barents_below100_no_ice_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/Barents_below100_no_ice_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/Barents_below100_no_ice_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/Barents_below100_no_ice_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/Barents_below100_no_ice_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/Barents_below100_no_ice_ordisurfs_top_corr.png"),
          width = 1000,
          height = 550)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/Barents_below100_no_ice_ordisurfs_man_sel_domean.png"),
          width = 1000,
          height = 1000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Barents_below100_no_ice_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
dca_int <- ggplot(data = env_vis,
  ...
  ggtitle("Interactive DCA sample ID plot",
          subtitle = "First run") +
  geom_point(aes(colour = factor(mud))) +
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/Barents_below100_no_ice_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
