...
## Data
```{r}
env_sort <- read.csv(file.path(dataPath,"inputs/ATLANTICSHELFhard_env_2022-10-24.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/ATLANTICSHELFhard_otu_2022-10-24.csv")) %>% as.data.frame
...
otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Tunicata_trunk")))]!=0)
...
otuSelSpR<-as.data.frame(rowSums(otuSel[,c(3:(which(colnames(otuSel)=="Tunicata_trunk")))]!=0))
summary(otuSelSpR)
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SHELF_HARD_ATL_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SHELF_HARD_ATL_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SHELF_HARD_ATL_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```{r}
otu_6$spRich<-rowSums(otu_6[,c(1:which(colnames(otu_6)=="Tunicata_trunk"))]!=0)
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_ordisurfs_man_sel_adjusted.png"),
          width = 1000,
          height = 800)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_EnvFit_man_sel_cln_adjusted.png"),
       device = "png",
       dpi=300 )
```
...
```{r}
ggexport(p_sed,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_gnmds_sedclass.png"),
          width = 1000,
          height = 800)
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
