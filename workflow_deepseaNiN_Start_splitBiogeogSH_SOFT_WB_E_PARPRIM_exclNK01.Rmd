...
abundTaxon<-otu_6 %>%
  pivot_longer(cols=!c(SampID, spRich,gnmds1, gnmds2,dca1,dca2), 
               names_to="Taxon", 
               values_to="Density",
                values_drop_na=T) %>%
  group_by(SampID) %>%
  slice(which.max(Density)) %>%
  mutate(DomTaxon=Taxon,
         DomTaxonDens=Density)%>%
  select(SampID, DomTaxon, DomTaxonDens)

env_sub_meta<-left_join(env_sub_meta,abundTaxon)
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  ggtitle("X v Y - coloured by gnmds r6 ax1") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__XvY_nmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  scale_colour_gradientn(limits = c(min(env_sub_meta$bathy),max(env_sub_meta$bathy)), #match variable acting as colour
                        colors=c('red','yellow','green'))+
  ggtitle("X v Y - coloured by bathy") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__XvY_bathy.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__XvY_biotope.png"), #amend
       device = "png",
       dpi=300 )

```

#### X v Y with dominant taxon
```{r}
cat_xy_ax1 <- ggplot(data = env_sub_meta,
              aes(x = X,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = env_sub_meta$DomTaxon), #change at will - categorical vars only
             size = 1) +
 scale_colour_manual(values=col_vector) +# non-ordered colourblind pallette
  # scale_colour_brewer(palette = "Spectral") + # ordered colourblind pallette
  ggtitle("Easting vs Northing - coloured by dominant taxon") #amend
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__XvY_domTaxon.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM_bathy_v_temp_nmds1.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM_bathy_gnmds_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM_bathy_dca_spR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__X_v_Y_tmeanR.png"), #amend
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__gnmds_gmo.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__XvY_ax1cat.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save some outputs
ggexport(comp,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_E_PARAPRIM__compare.png"), #amend
...
```{r}
#save.image("I:/Scripts/deepseaNiN/Renv_deepseaNiN_SH_SOFT_WB_E_PARAPRIM_.RData") # edit area first
