...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = gnmds1),
             size = 1) +
  scale_colour_gradientn(limits = c(-1.5, 3.3),
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = Smax_Robinson),
             size = 1) +
  scale_colour_gradientn(limits = c(34.5, 35.4),
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = X.y),
             size = 1) +
  scale_colour_gradientn(limits = c(205261, 364461),
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = Tmean_Robinson),
             size = 1) +
  scale_colour_gradientn(limits = c(-0.7, 8.2),
                        colors=c('red','yellow','green'))+
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = BO22_dissoxmean_bdmean),
             size = 1) +
scale_colour_gradientn(limits = c(285, 293),
                        colors=c('red','yellow','green'))+
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = Tmean_Robinson),
             size = 1) +
  scale_colour_gradientn(limits = c(-0.6, 8.2),
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = swDensRob_avs),
             size = 1) +
  scale_colour_gradientn(limits = c(1028, 1041),
                        colors=c('blue','green','red'))+
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = bathy),
             size = 1) +
  scale_colour_gradientn(limits = c(-1200, -400),
                        colors=c('red','yellow','green'))+
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/NSTmeanR_v_dissoxltmin_bathy_400to1200.png"),
       device = "png",
       dpi=300 )
...
### categorise gnmds Intermediate Values

```{r}
env_sub_meta$gnmdsInt<-cut(env_sub_meta$gnmds1, 
      breaks=c(-3.2,0.1, 1.6,3.46),
      labels=c("LoDens", "Intermediate", "HiDens"))


```


#### X v Y intermediate gnmds highlighted
```{r}
dis_split <- ggplot(data = env_sub_meta,
              aes(x = X.y,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = gnmdsInt),
             size = 1) +
 # scale_colour_manual(values=cbPalette) +# non-ordered colourblind pallette
  scale_colour_brewer(palette = "Paired") + # ordered colourblind pallette
  ggtitle("map coloured by categorical gnmds thresholds")

dis_split

summary(env_sub_meta$gnmdsInt)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/NS_XvY_intermediate.png"),
       device = "png",
       dpi=300 )

```
### categorise density Intermediate Values

```{r}
env_sub_meta$densCat<-cut(env_sub_meta$swDensRob_avs, 
      breaks=c(1028,1031.29, 1031.81,1041),
      labels=c("LoDens", "Int 1031.3-1031.8", "HiDens"))
...
dis_split <- ggplot(data = env_sub_meta,
              aes(x = X.y,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = densCat),
             size = 1) +
 # scale_colour_manual(values=cbPalette) +# non-ordered colourblind pallette
  scale_colour_brewer(palette = "Paired") + # ordered colourblind pallette
  ggtitle("map coloured by categorical density thresholds")
