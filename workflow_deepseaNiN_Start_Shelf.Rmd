...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/LDnoSBlo02_env_sort_2022-09-29.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/LDnoSBlo02_otu_sort_2022-09-29.csv")) %>% as.data.frame

###############
# Removing bad sites based on depth and cruise number. 
#Might move down to be included in the map of excluded sites
env_sort<-env_sort[env_sort$cruise_no != "2006612",]
otu_sort<-otu_sort[otu_sort$SampID %in% env_sort$SampID,]
...
Make function You might have to drop variables that have been imported
as character
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/Shelf_without2006_geodist_pa.rds")))
```
...
#Jonatan's upgrade for speed - parallel processing of mds repetitions
library(purrr)
...
```{r}
saveRDS(Xmds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/Shelf_without2006_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/Shelf_without2006_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
```{r}
ggsave(filename = file.path(dataPath,"outputs/Shelf_without2006_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/Shelf_without2006_geodist_r6.rds")))
```
...
```{r}
saveRDS(Xmds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/Shelf_without2006_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/Shelf_without2006_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```

\#### 1000 reps - don't run if loading saved object Currently commented
out as it gained nothing but took extra time. Can be removed in due
course. <!-- took 1hr --> <!-- ```{r} --> <!-- # monoMDS -->
<!-- d <- 2 --> <!-- mds_r6_1000 <- list()#obj added_1000 -->
...
```{r}
ggsave(filename = file.path(dataPath,"outputs/Shelf_without2006_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
```{r}
ggsave(file.path(dataPath,"outputs/Shelf_without2006_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/Shelf_without2006_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/Shelf_without2006_ordisurfs_top_corr.png"),
          width = 1000,
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/Shelf_without2006_ordisurfs_man_sel_domean.png"),
          width = 2000,
          height = 2000)
...
```{r}
ggsave(filename = file.path(dataPath,"outputs/Shelf_without2006_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/Shelf_without2006_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
...
Adding new columns to check patterns

```{r}
Spong_Spick<-otu_sort[,c('SampID','Geodia.Stelleta', 'Geodia.Stryphnus' ,'Geodia_atlantica' , 'Geodia_barretti', 'Geodia_macandrewii', 'Geodia_phlegraei', 'Geodia_sp.', 'Steletta_grubei','Stelletta_sp.', 'Stryphnus_ponderosus','Porifera_big', 'Hexadella_dedritifera')]
Spong_Spick$SpickSpong_Sum<-rowSums(Spong_Spick[,2:9])
env_vis_temp<-env_vis[,2:ncol(env_vis)]
env_vis_temp<-cbind(env_vis_temp, Spong_Spick) #
Lophelia_pertusa<-otu_6[,c('Lophelia_pertusa')] 
env_vis_temp<-cbind(env_vis_temp, Lophelia_pertusa) #
Lophelia_Paragorgia<-otu_6[,c('Lophelia_pertusa', 'Paragorgia_arborea')] 
Lophelia_Paragorgia<-rowSums(Lophelia_Paragorgia)
env_vis_temp<-cbind(env_vis_temp, Lophelia_Paragorgia)#

cruise_no<-env_sort$cruise_no
env_vis_temp<-cbind(env_vis_temp, cruise_no)#
...
```{r}
env_sel_locations<- ggplot(data = env_vis_temp,
                      aes(x = X.y,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = dca2*dca1),
             size = 2, alpha = 0.5) +
  scale_colour_gradient2(low = "green",
                         mid = "yellow",
                         high = "red") +
  ggtitle("Location of dca extremes")
...
p_dca_plotly <- ggplot(data = env_vis_temp,
                     aes(x = dca1,
                         y = dca2)) +
  theme_classic() +
  coord_fixed() +
...
ggplot(data = env_vis_temp,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by Spickle sponges density",
          subtitle = "First run") +
  geom_point(color = "grey") +
  geom_point(data =env_vis_temp[!is.na(env_vis_temp$SpickSpong_Sum) & env_vis_temp$SpickSpong_Sum>0,],  aes(colour = SpickSpong_Sum), alpha = 0.3) +
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
ggplot(data = env_vis_temp,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("DCA coloured by Spickle sponges density",
          subtitle = "First run") +
  geom_point(color = "grey") +
  geom_point(data =env_vis_temp[!is.na(env_vis_temp$SpickSpong_Sum) & env_vis_temp$SpickSpong_Sum>0,],  aes(colour = log(SpickSpong_Sum)), alpha = 0.3) +
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
ggplot(data = env_vis_temp,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by Lophelia density",
          subtitle = "First run") +
  geom_point(color = "grey") +
  geom_point(data =env_vis_temp[!is.na(env_vis_temp$Lophelia_pertusa) & env_vis_temp$Lophelia_pertusa>0,],  aes(colour = Lophelia_pertusa), alpha = 0.3) +
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
ggplot(data = env_vis_temp,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("DCA coloured by Lophelia density",
          subtitle = "First run") +
  geom_point(color = "grey") +
  geom_point(data =env_vis_temp[!is.na(env_vis_temp$Lophelia_pertusa) & env_vis_temp$Lophelia_pertusa>0,],  aes(colour = Lophelia_pertusa), alpha = 0.3) +
  geom_vline(xintercept = 0,
...
env_sel_locations_Lophelia_Paragorgia<- ggplot(data = env_vis_temp,
                      aes(x = X.y,
                          y = Y)) +
  theme_classic() +
  geom_point(data = env_vis_temp[env_vis_temp$Lophelia_Paragorgia == 0,],colour = "gray",
             size = 2, alpha = 0.5) +
   geom_point(data = env_vis_temp[env_vis_temp$Lophelia_Paragorgia > 0,], aes(colour = Lophelia_Paragorgia),
             size = 2, alpha = 0.5) +
  scale_colour_gradient2(low = "green",
                         mid = "yellow",
                         high = "red") +
...
map_gnmds1<- ggplot(data = env_vis_temp,
                      aes(x = X.y,
                          y = Y)) +
  theme_classic() +
...
env_sel_locations_Lophelia_Paragorgia + map_gnmds1
