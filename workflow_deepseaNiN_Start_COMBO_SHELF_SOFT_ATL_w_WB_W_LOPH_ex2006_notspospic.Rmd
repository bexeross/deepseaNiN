---
title: "MAREANO - NiN - COMBO CHECK - SHELF SOFT ATLANTIC with WEST BARENTS WEST (Lophelia extent) excluding 2006 cruise and not sponge spicule bottom"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__mds_r6_1000rep.rds"))
...
### LOAD AND DEAL WITH THE WB W DATA 
i.e. make the WB W excl outliers and 2006

```{r}
# LOAD WB W DATA
env_wbw <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SH_WB_W_LOPH_soft_env_2022-10-28.csv")) %>% as.data.frame
otu_wbw <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SH_WB_W_LOPH_soft_otu_2022-10-28.csv")) %>% as.data.frame


table(env_wbw$SampID2 == otu_wbw$SampID)
...
#### REMOVE NK01 and 2006 samples
```{r}
# remove NK01 samples
env_filt<-filter(env_wbw, VL<1774 | VL>1784)
nk01_check<-filter(env_wbw, VL>=1774 & VL<=1784)

### check what removing
plot(env_filt$X.y, env_filt$Y, ylim=c(min(env_filt$Y), max(env_filt$Y)+100000))
points(nk01_check$X.y, nk01_check$Y, col='red')

#------------
# remove 2006 data
env_filt2<-filter(env_filt, !str_detect(cruise_no,'2006612'))
cr2006_check<-filter(env_filt, (str_detect(cruise_no,'2006612')))

### check what removing
plot(env_filt2$X.y, env_filt2$Y, ylim=c(min(env_filt2$Y), max(env_filt2$Y)+100000))
points(cr2006_check$X.y, cr2006_check$Y, col='red')

#-----------
# remove sponge spic samples
...
#      TRUE~"other"))
# 
# notSpSpicSamp<-filter(biotopeInfo2, str_detect(SpongeSpic,'other'))
# 
# 
# env_filt2<-filter(env_filt, (SampID %in% notSpSpicSamp$SampID))
# SpSpic_check<-filter(env_filt, !(SampID %in% notSpSpicSamp$SampID ))
#
#
# ### check what removing
# plot(env_filt2$X.y, env_filt2$Y, ylim=c(min(env_filt2$Y), max(env_filt2$Y)+100000))
# points(SpSpic_check$X.y, SpSpic_check$Y, col='red')


#-----------
# remove outliers

env_filt3<-env_filt2 %>% filter(!str_detect(SampID, "439_02|390_02")) %>% mutate("dataset"="WBWx2006")
outl_check<-env_filt2 %>% filter(str_detect(SampID, "439_02|390_02"))

### check what removing
plot(env_filt3$X.y, env_filt3$Y, ylim=c(min(env_filt3$Y), max(env_filt3$Y)+100000))
points(outl_check$X.y, outl_check$Y, col='red')

#ensure env and otu datasets have same samples in same order
joindat_wbwx2006<-left_join(env_filt3, otu_wbw, by ='SampID')
#env_wbwx2006<-joindat %>% select(c(1:which(colnames(joindat)=="X.y.y")-1)) #dataset before 19-10-22
#otu_wbwx2006<-joindat %>% select(c("X.y.y",
#                               "SampID",
#                               c(which(colnames(joindat)=="X.y.y"):which(colnames(joindat)=="Tubularia_sp."))))#dataset before 19-10-22
```
...
```{r}
# LOAD WB W DATA
env_atl <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/ATLANTICSHELFsoft_env_2022-10-24.csv")) %>% as.data.frame  %>%
  mutate("dataset"="ATLANTIC")
otu_atl <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/ATLANTICSHELFsoft_otu_2022-10-24.csv")) %>% as.data.frame


table(env_atl$SampID2 == otu_atl$SampID)
...
joindat_atl<-left_join(env_atl, otu_atl, by ='SampID')
```

## JOIN wbwx2006 and ATL datasets

```{r}
joindat<-bind_rows(joindat_wbwx2006, joindat_atl) 

env_sort<-joindat %>% select(c(1:which(colnames(joindat)=="X.y.y")-1))
otu_sort<-joindat %>% 
  select(c("X.y.y","SampID",c(which(colnames(joindat)=="X.y.y"):which(colnames(joindat)=="Terebellida")))) %>%
  select(c("X.y.y","SampID"), sort(colnames(.))) %>%
  select_if(~ !any(is.na(.))) # we cannot keep any taxa which are NA in the other area
...
```{r}
# lightvars<-env_sort %>% select(c("SampID",
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```{r}
env_cont<-env%>% select(-c(landscape,sedclass,gmorph, MLDmean_bathy, MLDmax_bathy, MLDmin_bathy, dataset, #categorical
                           optional, #not a var
                           ...
                           vClayOutcrop, vClay, vBiobush, vConsolidated_clay, vMud, #too many NAs
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/COMBO_SH_SOFT_ATL_WBWx2006__cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
cor_sel<-subset(cor_a2_sort,abs_ord_ax2>cor_cut)
cor_sel
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)
...
env_os_m <- env[,c("Tmean_Robinson", #top corr
                  ...
                  "X.y", #comparison to Y
                  "Y", #top corr
                  "CSpdmean_Robinson", #comparison to top corr ax2 (blended model)
                  "CSpdmax_Robinson", #comparison to top corr ax2 (blended model)
                  "CSpdsd_Robinson", #comparison to top corr ax2 (blended model)
                  "spd_mean",
                  "spd_max",
                  "spd_std",
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__ordisurfs_man_sel_adjusted.png"),
          width = 800,
          height = 1500)
...
env_os_m_envfit <- env[,c("Tmean_Robinson", #top corr
                  "Smax_Robinson", #comparison to top corr
                  "swDensRob_avs", #top corr
                  "BO22_dissoxmean_bdmean",#top corr
                  "X.y", #comparison to Y
                  ...
                  "vSand",
                  "vSandy_mud",
                  "bathy" #intuitive for comparisonss
                )]


colnames(env_os_m_envfit)<-c("T", #top corr
                  "S", #comparison to top corr
                  "Dens", #top corr
                  "O2",#top corr
                  "X", #comparison to Y
                  "Y", #top corr
                  "CSmean", #comparison to top corr ax2 (blended model)
                  "CSmax", #comparison to top corr ax2 (blended model)
                  "CSsd", #comparison to top corr ax2 (blended model)
                  "spd_mean",
                  "spd_max",
                  "spd_std",
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__EnvFit_man_sel_cln_adjusted.png"),
       device = "png",
       dpi=300 )
```
...
p_sed <- ggplot(data = env_vis,
  ...
  scale_colour_manual(values=c("Biogenic"="darkslateblue",# some types not yet listed
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/COMBO_SH_SOFT_ATL_WBWx2006__gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
...
#### gnmds w dataset
```{r}
dca_ds <- ggplot(data = env_vis,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("Interactive gnmds with dataset labels",
          subtitle = "First run") +
