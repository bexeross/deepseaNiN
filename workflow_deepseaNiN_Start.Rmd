...
```{r}
envCompl_ccrem<-envCompl%>%filter(!SampID%in%envSel$SampID)
...
```{r}
env<-env%>% mutate ("MLDmean-bathy"=MLDmean_Robinson-(bathy*-1),
                    "MLDmin-bathy"=MLDmin_Robinson-(bathy*-1),
                    "MLDmax-bathy"=MLDmax_Robinson-(bathy*-1))
```
...
```{r}
env_cont<-env%>% select(-c(landscape,sedclass,gmorph, #categorical
                           optional, #not a var
...
dotchart(cor_a1_sort$abs_ord_ax1)

cor_cut<-0.333 #decide
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  scale_colour_gradient2(low = "red",
                         mid = "yellow",
                         high = "green") +
  ggtitle("Geography (X v Y) - coloured by gnmds r6 200 rep axis 1")
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/X_v_Y_nmds1.png"),
       device = "png",
       dpi=300 )
```
##### Temp v disoxltmin
...
tb_ax1<- ggplot(data = env_sub_meta,
  ...
  scale_colour_gradient2(low = "red",
                         mid = "yellow",
                         high = "green") +
  ggtitle("Temp (mean Robinson) v disox lt min - coloured by depth")
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/TmeanR_v_dissoxltmin_depth.png"),
       device = "png",
       dpi=300 )
```
