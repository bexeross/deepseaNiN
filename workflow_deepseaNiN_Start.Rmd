...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"geodist_r6.rds")))
```
...
png(file.path(dataPath,"procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
saveRDS(mds_r6_1000, #obj added_1000
        file = file.path(dataPath,"mds_r6_1000rep.rds")) #change from 200
```
...
png(file.path(dataPath,"procrustes_r6_1000.png"), width=1000, height=700,) #added 1000
plot(procr_r6_1000)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"ordisurfs_top_corr.png"),
          width = 2000,
          height = 2000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"ordisurfs_man_sel.png"),
          width = 2000,
          height = 1000)
