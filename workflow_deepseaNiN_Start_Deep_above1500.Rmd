---
title: "MAREANO - NiN - hiDens, above 1500m"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_mds_r6_1000rep.rds"))
...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/DEEP/DeepAbove1500_env_sort_2022-09-29.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/DEEP/DeepAbove1500_otu_sort_2022-09-29.csv")) %>% as.data.frame
...
#str(envSel)
```
Here we should remove any samples that are excluded, instead of code that is later on.
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/hiDens_a1500_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/hiDens_a1500_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
### DCA R6
```{r}
#remove extra columns from previous run
#otu_6 <- otu_6[,-c(388:391)]
...
##get sample id for outliers
```{r}
#otu_6_sampID <- cbind(otu_6,otuSel$SampID)

```

30.9.22 remove dca >1 and <-1 from otu_6 before rerun
```{r}

# otu_6 <- otu_6_sampID[!abs(-otu_6_sampID$dca1)>1,]
# 
# #remove sampleID
# otu_6 <- otu_6[ , !(names(otu_6) %in% "otuSel$SampID")]
```
running again without the outliers
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/hiDens_a1500_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
<!--         file = file.path(dataPath,"inputs/SPLITS/DEEP/mds_r6_1000rep.rds")) #change from 200 -->
<!-- ``` -->
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/hiDens_a1500_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/hiDens_a1500_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
want to remove outliers >1 and <-1 in DCA to make patterns clearer in the core.
...
need to remove the outliers from env as well

<!-- ```{r} -->
<!-- env_corr_sampID <- cbind(env_corr,otuSel$SampID) -->
<!-- otu_6_rm <- otu_6_sampID[abs(-otu_6_sampID$dca1)>1,] -->
<!-- rm <- otu_6_rm$`otuSel$SampID` -->
<!-- rm -->

<!-- #remove the rows with values from the list of removed samples -->
<!-- env_corr <- env_corr_sampID[ ! env_corr_sampID$`otuSel$SampID` %in% rm, ] -->

<!-- #remove sampleID -->
<!-- env_corr <- env_corr[ , !(names(env_corr) %in% "otuSel$SampID")] -->
<!-- ``` -->

remove the removed samples from env. so we can make env_os with correct dimensions
```{r}
env_sampID <- cbind(env,otuSel$SampID)
otu_6_rm <- otu_6_sampID[abs(-otu_6_sampID$dca1)>1,]
rm <- otu_6_rm$`otuSel$SampID`
rm

#remove the rows with values from the list of removed samples
env <- env_sampID[ ! env_sampID$`otuSel$SampID` %in% rm, ]

#remove sampleID
env <- env[ , !(names(env) %in% "otuSel$SampID")]
```
#### Correlations
```{r}
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_a1500_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/hiDens_a1500_ordisurfs_top_corr.png"),
          width = 2000,
          height = 2500)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/hiDens_a1500_ordisurfs_man_sel_domean.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/hiDens_a1500_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/hiDens_a1500_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
