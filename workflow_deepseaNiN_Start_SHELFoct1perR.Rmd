---
title: "MAREANO - NiN - SHELF october 2022 - 1 per R station"
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_mds_r6_1000rep.rds"))
...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/LDnoSBlo02_env_sort_2022-10-13.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/LDnoSBlo02_otu_sort_2022-10-13.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SHELFoct1perR_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFoct1perR_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SHELFoct1perR_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFoct1perR_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SHELFoct1perR_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFoct1perR_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SHELFoct1perR_ordisurfs_top_corr.png"),
          width = 800,
          height = 1500)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SHELFoct1perR_ordisurfs_man_sel_domean.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFoct1perR_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SHELFoct1perR_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
