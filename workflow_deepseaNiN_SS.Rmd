---
title: "MAREANO - NiN: Svalbard Slope Run"
...
env_sort <- read.csv(file.path(dataPath,"inputs/SSenv_sort_2022-09-23.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SSotu_sort_2022-09-23.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SSgeodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SSmds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SSprocrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SScorrelationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SSgeodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SSmds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SSprocrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SScorrelationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SSgnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SScor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SSordisurfs_top_corr.png"),
          width = 2000,
          height = 2000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SSordisurfs_man_sel.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SSEnvFit_man_sel_cln.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SSgnmds_MLDmeanBathy.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w sedclass
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SSgnmds_sedclass.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w lanscape
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SSgnmds_landscape.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SSgnmds_gmorph.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SSgnmds_catvar.png"),
          width = 1000,
          height = 800)
```
...
T.mds<- ggplot(data = env_vis,
  ...
  ggtitle("Gnmds coloured by temp (SS)")
...
```{r}
dotchart(env_vis$Tmean_Robinson, main="Dotchart of TmeanR at Norland Slope")
```

### Dotplot zoomed on 2-4*C
```{r}
dotchart(env_vis$Tmean_Robinson, xlim=c(2,4),main="Dotchart of TmeanR between 2 and 4*C at Norland Slope")
```