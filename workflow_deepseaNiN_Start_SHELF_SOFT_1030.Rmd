---
title: "MAREANO - NiN - SHELF - SOFT density below 1030 kg /m3"
...
NB the SHELF SOFT analysis is being done on all samples (not 1perR) and is here being redone after realising in SHELF_SOFToct that the density threshold that cut off the deep left too much of the AIW in the shelf dataset. New 1030 kg/ m3 threshold is applied in this run to remove those deep outliers.
...
library(GGally)
library(ggrepel)
library(ggordiplots) #find installation advice here https://rdrr.io/github/jfq3/ggordiplots/
#library(readr)
...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_mds_r6.rds"))
#mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_mds_r6_1000rep.rds"))
...
env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_env_2022-10-21.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_otu_2022-10-21.csv")) %>% as.data.frame
...
Sediment miljø (erosion/depositon stuff) https://www.ngu.no/upload/Kartkatalog/Presentasjonsregler_Marin_Sedimentasjonsmiljo.pdf
Sediment dannelse (gensis) https://www.ngu.no/upload/Kartkatalog/Presentasjonsregler_Marin_SedimentDannelse.pdf
```{r}
...
envExtra<-envExtra %>% select(c("SampID2", 
                                ...
                                "TOC0.10cm_median" #Total Organic Carbon (Markus Diesing model 10/2022)
))
...
```{r}
otu<-otuSel[,-c(1:which(colnames(otuSel)=="Asbestopluma_pennatula")-1)]
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SHELFsoft_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFsoft_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SHELFsoft_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFsoft_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SHELFsoft_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```

## Check outliers in DCA with gnmds axis 1
```{r}
...
p_spR <- ggplot(data = otu_6,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("DCA coloured by gnmds1",
          subtitle = "First run") +
  geom_point(aes(colour = gnmds1)) +
  scale_colour_gradientn(limits = c(min(otu_6$gnmds1),max(otu_6$gnmds1)),
...
## Check outliers in DCA species Richness
```{r}
p_spR <- ggplot(data = otu_6,
                     aes(x = dca1,
                         y = dca2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("DCA coloured by species Richness",
          subtitle = "First run") +
...
env_cont<-env%>% select(-c(landscape,sedclass,gmorph, MLDmean_bathy, MLDmax_bathy, MLDmin_bathy, #categorical
                           ...
                           vClayOutcrop, vClay, vBiobush, vConsolidated_clay,
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELFsoft_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SHELFsoft_ordisurfs_top_corr.png"),
          width = 800,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SHELFsoft_ordisurfs_man_sel_domean.png"),
          width = 1000,
          height = 1000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELFsoft_EnvFit_man_sel_cln_domean.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SHELFsoft_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
