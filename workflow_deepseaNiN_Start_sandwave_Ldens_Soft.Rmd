---
title: "MAREANO - NiN - sandwave lodensity, soft"
...
2.11.22 soft fauna only in this script

```{r}

env_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_soft_env2022-11-02.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_LDens_soft_otu2022-11-02.csv")) %>% as.data.frame
...
dim(otu_sort)
```
### Round 2 - Remove outliers identified in round 1 
```{r}
#remove 439_02
env_sort<-env_sort %>% 
  filter(!grepl('439_02', SampID))
...
## Removing observations with less than 13 OTUs

otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Urasterias_lincki")))]!=0) #CHANGE TO LAST OCCURING SPECIES IN DATASET
...
#### Plot map to check Landscape
```{r}
env_sort_SampID<- ggplot(data = envSel,
                      aes(x = X.y,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = factor(landscape)), size=1) +
  ggtitle("Location of samplesIDs in sorted env file")
...
```{r}
otu<-otuSel[,-c(1:which(colnames(otuSel)=="Actiniaria.yellow")-1)] #CHANGE TO 1st SPECIES IN DATASET
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_SOFT_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_SOFT_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/sandwave_Ldens_SOFT_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_SOFT_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_SOFT_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/sandwave_Ldens_SOFT_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/sandwave_Ldens_SOFT_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```{r}
otu_6$spRich<-rowSums(otu_6[,c(1:which(colnames(otu_6)=="Urasterias_lincki"))]!=0)
...
```
```{r}
ggsave(file.path(dataPath,"outputs/sandwave_Ldens_SOFT_gnmds_SpRichr6.png"),
       device = "png", 
       dpi=300)
```
...
## Check outlier SampIDs
```{r}
d_gmo <- ggplot(data = otu_6,
                     aes(x = dca1,
                         y = dca2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("DCA coloured by sample",
          subtitle = "First run") +
  geom_point(aes(colour = factor(SampID))) +
  geom_vline(xintercept = 0,
             linetype = 2,
...
Round 1:
Need to trim off outlying samples 439_02 and 390_02 - outliers at both GNMDS and DCA and also the only surving samples from those VLs - others have been filtered out.

Round 2: no major outliers
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/sandwave/sandwave_Ldens_SOFT_cor-table_r6_200rep.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)

```

Needs further splitting - not at local variation yet.
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_ordisurfs_man_sel.png"),
          width = 1000,
          height = 1000)
...
env_os_m_envfit<-env_os_m [,c(
                  # "BO22_chlorange_ss", #top corr
                  # "BO22_dissoxltmin_bdmean", #top corr
                  # "Y", #top corr
                  # "temp_max", #top corr
                  # "BO22_carbonphytoltmin_bdmean",#top corr
                  # "BO22_dissoxmean_bdmean",#top corr
                  # "temp_mean", #top corr
                  # "BO22_ppltmin_ss", #top corr
                  # "BO22_chloltmin_ss", #top corr
                  "mud", #manual
                  "gravel",#manual
                  "sw_spd_mean", #manual
                  "sw_spd_max", #manual
                  "spd_mean", #manual
                  "spd_max", #manual
                  "CSpdmean_Robinson", #manual
                  "CSpdmax_Robinson",#manual
                  "TOC0.10cm_median"#manual
                  #"bathy" #intuitive for comparisons
)]

colnames(env_os_m_envfit)<-c(
                  # "chlorange", #top corr
                  # "dissoxltmin", #top corr
                  # "Y", #top corr
                  # "temp_max", #top corr
                  # "Cphytoltmin",#top corr
                  # "dissoxmean",#top corr
                  # "temp_mean", #top corr
                  # "ppltmin", #top corr
                  # "chloltmin", #top corr
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_EnvFit_man_sel.png"),
       device = "png",
       dpi=300 )
```
...
p_sed <- ggplot(data = env_vis,
  ...
  geom_point(aes(colour = factor(sedclassName))) +
    scale_colour_manual(values=c("Biogenic"="darkslateblue",
                               "gmSand"="darkgoldenrod3",
                               "GravBlock"="deeppink1",
                               "gSand"="goldenrod1",
                               "gsMud"="skyblue3",
                               "HardSed"="saddlebrown",
                               ...
                               "sGravel"="lightsalmon",
                               "sMud"="lightseagreen",
        "MwBlock"="steelblue" #added after Deep plotting
                               )) +
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
...
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_LoDens_SOFT_gnmds_sedclass.png"), #amend
       device = "png",
       dpi=300 )
...
```
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_LoDens_SOFT_gnmds_land.png"), #amend
       device = "png",
       dpi=300 )
...
env_vis<- env_vis %>%
  mutate(
    seddanName = case_when(
            seddan == "10" ~ "Moraine_indet", ########### ALL NEW NAMES NEED ADDING!
            seddan == "11" ~ "Moraine_thick",
            seddan == "15" ~ "Moraine_marginal",
            seddan == "140" ~ "Rock",
            ...
            seddan == "230" ~ "Moraine_glaciomarineCover",
...
p_dann <- ggplot(data = env_vis,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by sed dann",
          subtitle = "First run") +
  geom_point(aes(colour = factor(seddanName))) +
   scale_colour_manual(values=c("Morraine_indet"="yellowgreen",
                               "Morraine_thick"="green",
                               "Morraine_marginal"="darkgreen",
                               "Rock"="pink",
                               "Suspension"="lightblue",
                               "Bottom_current"="goldenrod1",
                               "Glaciomarine"="mediumblue",
                               "Debris_flow"="salmon",
                               "Bioclastic"="midnightblue",
                               "Carbonate_crust"="black",
                               "Morraine_glaciomarineCover"="aquamarine3",
                               "Slide_part_cover"="darkorchid1",
                               "Channel_deposit"="tan1",
                               "Contourite"="gold1",
                               "Layered_over_debris_flow"="tomato3",
                               "Slide_w_cover"="purple4",
                               "Sedimentary_rock"="saddlebrown"
...
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_LoDens_SOFT_gnmds_seddan.png"), #amend
       device = "png",
       dpi=300 )
...
env_vis<- env_vis %>%
  mutate(
    sedmilName = case_when(
            sedmil == "1" ~ "Deposition from suspension", ########### ALL NEW NAMES NEED ADDING!
            sedmil == "2" ~ "Deposition from suspension, local erosion of fine-grained sediments",
            sedmil == "7" ~ "No/very slow deposition",
            sedmil == "3" ~ "Deposition from bottom currents",
            sedmil == "4" ~ "Erosion, local deposition of sediments in topographic lows",
            sedmil == "5" ~ "Erosion"
    ))
```
...
p_sedmil <- ggplot(data = env_vis,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by sed env",
          subtitle = "First run") +
  geom_point(aes(colour = factor(sedmilName))) +
    scale_colour_manual(values=c("Deposition from suspension"="cornflowerblue",
                               "Deposition from suspension, local erosion"="skyblue",
                               "Deposition from bottom currents"="lightblue1",
                               "Erosion"="salmon",
                               "Erosion, local deposition"="lightsalmon",
                               "No/very slow deposition"="slateblue")) +
...
```
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_LoDens_SOFT_gnmds_sedmil.png"), #amend
       device = "png",
       dpi=300 )
...
env_vis<- env_vis %>%
  mutate(
    gmorphName = case_when(
            gmorph == "1" ~ "flat", ########### ALL NEW NAMES NEED ADDING!
            gmorph == "2" ~ "peak",
            gmorph == "3" ~ "ridge",
            gmorph == "4" ~ "shoulder",
            gmorph == "5" ~ "convex slope",
            gmorph == "6" ~ "slope",
            gmorph == "7" ~ "concave slope",
            gmorph == "8" ~ "footslope",
            gmorph == "9" ~ "valley",
            gmorph == "10" ~ "pit"
    ))
...
p_gmo <- ggplot(data = env_vis,
  ...
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by geomorphon class",
          subtitle = "First run") +
...
##### Save the plot
```{r}
ggexport(morph_plots,
          filename = file.path(dataPath,"outputs/sandwave_Ldens_SOFT_gnmds_gmorph.png"),
          width = 1000,
          height = 800)
```
