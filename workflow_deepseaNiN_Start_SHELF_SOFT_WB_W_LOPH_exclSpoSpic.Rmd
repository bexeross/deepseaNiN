---
title: "MAREANO - NiN - SHELF SOFT WEST BARENTS WEST (Lophelia extent) excluding SPONGE SPICULE BOTTOM"
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_ordisurfs_man_sel_adjusted.png"),
          width = 1000,
          height = 800)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_EnvFit_man_sel_cln_adjusted.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SH_SOFT_WB_W_exclSPOSPIC_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
