---
title: "MAREANO - NiN - loDens <1031.8"
...
env_sort <- read.csv(file.path(dataPath,"inputs/LDensShlw_env_sort_2022-09-23.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/LDensShlw_otu_sort_2022-09-23.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/LoDens_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/LoDens_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/LoDens_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/LoDens_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/LoDens_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/LoDens_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/LoDens_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/LoDens_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/LoDens_ordisurfs_top_corr.png"),
          width = 2000,
          height = 2500)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/LoDens_ordisurfs_man_sel.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_EnvFit_man_sel_cln.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_gnmds_MLDmeanBathy.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w sedclass
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_gnmds_sedclass.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w lanscape
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_gnmds_landscape.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/LoDens_gnmds_gmorph.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/LoDens_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
