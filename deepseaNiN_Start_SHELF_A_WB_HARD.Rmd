---
title: "MAREANO - NiN - SHELF ATLANTIC + WEST BARENTS WEST HARD BOTTOM TAXA"
authors: Thijs van Son, Rune Halvorsen, Rebecca Ross, Genoveva Gonzales-Mirelis, Margaret Dolan (updated 18.11.22)
...
<!-- ```{r} -->
<!-- geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_geodist_pa.rds")) -->
<!-- geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_geodist_r6.rds")) -->

<!-- mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_mds_pa.rds")) -->
<!-- mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_mds_r6.rds")) -->
<!-- #mds_r6_1000<-readRDS(file.path(dataPath,"inputs/SPLITS/SHELF/SH_SOFT_WB_W_exclSPOSPIC_mds_r6_1000rep.rds")) -->
...
env_orig_WBW <- read.csv(file.path(dataPath,"inputs/SH_WB_W_LOPH_hard_env_2022-10-28.csv")) %>% as.data.frame
otu_orig_WBW <- read.csv(file.path(dataPath,"inputs/SH_WB_W_LOPH_hard_otu_2022-10-28.csv")) %>% as.data.frame

env_orig_A <- read.csv(file.path(dataPath,"inputs/ATLANTICSHELFhard_env_2022-10-24.csv")) %>% as.data.frame
otu_orig_A <- read.csv(file.path(dataPath,"inputs/ATLANTICSHELFhard_otu_2022-10-24.csv")) %>% as.data.frame

# #check colnames are identical
# colnames(env_orig_A) %in% colnames(env_orig_WBW)
# colnames(otu_orig_A) %in% colnames(otu_orig_WBW)
# 
# #check rownames
# rownames(env_orig_A) %in% rownames(env_orig_WBW)
# rownames(otu_orig_A) %in% rownames(otu_orig_WBW)

#combine the 2 datasets - used bind_rows as different number cols for otu
env_sort <- bind_rows(env_orig_A, env_orig_WBW)
otu_sort <- bind_rows(otu_orig_A, otu_orig_WBW)
...
<!-- ### REMOVE NK01 and Sponge Spic samples -->
<!-- ```{r} -->
<!-- # remove NK01 samples -->
<!-- env_filt<-filter(env_orig, VL<1774 | VL>1784) -->
<!-- nk01_check<-filter(env_orig, VL>=1774 & VL<=1784) -->
...
<!-- env_filt3<-env_filt2 %>% filter(!str_detect(SampID, "439_02|390_02")) -->
...
<!-- joindat<-left_join(env_filt3, otu_orig, by ='SampID') -->
<!-- env_sort<-joindat %>% select(c(1:which(colnames(joindat)=="X.y.y")-1)) #dataset before 19-10-22 -->
<!-- otu_sort<-joindat %>% select(c("X.y.y", -->
<!--                                "SampID", -->
<!--                                c(which(colnames(joindat)=="X.y.y"):which(colnames(joindat)=="Tubularia_sp."))))#dataset before 19-10-22 -->


<!-- ``` -->
##remove samples from station 439_ previously identified as outliers in soft analysis and also causing problems in procrustes in first run here 
```{r}
#remove 439_
env_sort<-env_sort %>% 
  filter(!grepl('439_', SampID))
...
#set NAs in OTU resulting from table combination to 0 
otuCompl[is.na(otuCompl)] <- 0

## Removing observations with less than 4 OTUs

otuCompl$SpR<-rowSums(otuCompl[,c(3:(which(colnames(otuCompl)=="Porifera_lily")))]!=0)
...
otuSelSpR<-as.data.frame(rowSums(otuSel[,c(3:(which(colnames(otuSel)=="Porifera_lily")))]!=0))
summary(otuSelSpR)
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_WBW_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_WBW_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_WBW_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_WBW_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
```{r}
otu_6$spRich<-rowSums(otu_6[,c(1:which(colnames(otu_6)=="Porifera_lily"))]!=0)
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/SHELF/SHELF_HARD_ATL_WBW_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_ordisurfs_top_corr.png"),
          width = 1000,
          height = 1000)
...
env_os_m <- env[,c("Tmean_Robinson", 
                  "Smean_Robinson", 
                  "swDensRob_avs", #LKM DL
                  "BO22_dissoxmean_bdmean",#top corr
                  "X.y", #comparison to Y
                  "Y", #top corr
                  "CSpdmean_Robinson", #LKM VF
                  "CSpdmax_Robinson", #LKM VF
                  "CSpdsd_Robinson", #LKM VF
                  "spd_mean",#LKM VF
                  "spd_max",#LKM VF
                  "spd_std",#LKM VF
                  "mud", 
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_ordisurfs_man_sel_adjusted.png"),
          width = 1000,
          height = 800)
...
env_os_m_envfit <- env[,c("Tmean_Robinson", 
                  "Smean_Robinson", 
                  "swDensRob_avs", #LKM DL
                  "BO22_dissoxmean_bdmean",#top corr
                  "X.y", #comparison to Y
...
colnames(env_os_m_envfit)<-c("T_rob", 
                  "S_rob", 
                  "Dens", #LKM DL
                  "dissoxmean",#top corr
                  "X", #comparison to Y
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_EnvFit_man_sel_cln_adjusted.png"),
       device = "png",
       dpi=300 )
```
...
```{r}
ggexport(p_sed,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_gnmds_sedclass.png"),
          width = 1000,
          height = 800)
```
...
```{r}
cat_var_plots<-p_sed+p_land
```
##### Save the plot
```{r}
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/SHELF_HARD_ATL_WBW_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
