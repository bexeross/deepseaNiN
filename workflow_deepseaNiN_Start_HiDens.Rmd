---
title: "MAREANO - NiN - hiDens >1031.8"
...
env_sort <- read.csv(file.path(dataPath,"inputs/HDensDeep_env_sort_2022-09-23.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/HDensDeep_otu_sort_2022-09-23.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/HiDens_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/HiDens_mds_pa.rds"))
```
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/HiDens_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/HiDens_mds_r6.rds")) 
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/HiDens_cor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
### Dot chart to check for gaps in correlation - AXIS 2

```{r}

dotchart(cor_a2_sort$abs_ord_ax2, main="Absolute (+/-) correlations between envVars and gnmds axis 2")

cor_cut_a2<-0.26 #decide

cor_sel_a2<-subset(cor_a2_sort,abs_ord_ax2>cor_cut_a2)
cor_sel_a2
as.data.frame(cor_sel_a2$env)
```


### Sel env var (top corr) ax1
```{r}
env_os_a2 <- env[, cor_sel_a2$env]
env_os_a2
str(env_os_a2)
...
### Ordisurfs top corr ax 1
```{r}
ordsrfs_a2 <- list(length = ncol(env_os_a2))

for (i in seq(ncol(env_os_a2))) {
  os.i_a2 <- gg_ordisurf(ord = ord,
                      env.var = env_os_a2[, i],
                      pt.size = 1,
                      # binwidth = 0.05,
                      var.label = names(env_os_a2)[i],
                      gen.text.size = 10,
                      title.text.size = 15,
                      leg.text.size = 10)
  
  ordsrfs_a2[[i]] <- os.i_a2$plot
}

ordsrfs_plt_a2 <- ggarrange(plotlist = ordsrfs_a2,
                         nrow = 4,
                         ncol = 3)
...
##### Save some outputs
```{r}
ggexport(ordsrfs_plt_a2,
          filename = file.path(dataPath,"outputs/HiDens_ordisurfs_top_corr_a2.png"),
          width = 1000,
          height = 1000)
...
env_os_m <- env[,c("Tmean_Robinson", #top corr
                  ...
                  "cobB" #added as important, compare to mud /grav
                )]
env_os_m
str(env_os_m)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/HiDens_ordisurfs_man_sel.png"),
          width = 1000,
          height = 1500)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens_EnvFit_man_sel_cln.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens_gnmds_MLDmeanBathy.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w sedclass
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens_gnmds_sedclass.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w lanscape
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens_gnmds_landscape.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/HiDens_gnmds_gmorph.png"),
       device = "png",
       dpi=300 )
```
