---
title: "MAREANO - NiN: Finnmark Coast Run"
...
env_sort <- read.csv(file.path(dataPath,"inputs/FCenv_sort_2022-09-22.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/FCotu_sort_2022-09-22.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/FCgeodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/FCmds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/FCprocrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCcorrelationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/FCgeodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/FCmds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/FCprocrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCcorrelationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/FCgnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/FCcor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/FCordisurfs_top_corr.png"),
          width = 2000,
          height = 2000)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/FCordisurfs_man_sel.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCEnvFit_man_sel_cln.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCgnmds_MLDmeanBathy.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w sedclass
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCgnmds_sedclass.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w lanscape
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCgnmds_landscape.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/FCgnmds_gmorph.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/FCgnmds_catvar.png"),
          width = 1000,
          height = 800)
```
