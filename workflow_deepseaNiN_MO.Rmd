---
title: "MAREANO - NiN: MAREANO Øst Run"
...
env_sort <- read.csv(file.path(dataPath,"inputs/MOenv_sort_2022-09-22.csv")) %>% as.data.frame
otu_sort <- read.csv(file.path(dataPath,"inputs/MOotu_sort_2022-09-22.csv")) %>% as.data.frame
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/MOgeodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/MOmds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/MOprocrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOcorrelationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/MOgeodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/MOmds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/MOprocrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOcorrelationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/MOgnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/MOcor-table_r6_200rep_MLD-bathy.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/MOordisurfs_top_corr.png"),
          width = 2000,
          height = 1500)
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/MOordisurfs_man_sel.png"),
          width = 2000,
          height = 2000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOEnvFit_man_sel_cln.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOgnmds_MLDmeanBathy.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w sedclass
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOgnmds_sedclass.png"),
       device = "png",
       dpi=300 )
```
#### gnmds w lanscape
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOgnmds_landscape.png"),
       device = "png",
       dpi=300 )
```
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/MOgnmds_gmorph.png"),
       device = "png",
       dpi=300 )
```
...
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/MOgnmds_catvar.png"),
          width = 1000,
          height = 800)
```
