...
xy_t<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = BO22_dissoxmean_bdmean),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_sub_meta$BO22_dissoxmean_bdmean), max(env_sub_meta$BO22_dissoxmean_bdmean)), #match variable acting as colour
                        colors=c('red','yellow','green'))+
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_1030__X_v_Y_dissoxmean.png"), #amend
       device = "png",
       dpi=300 )
```
...
xy_t<- ggplot(data = env_sub_meta,
  ...
  geom_point(aes(colour = BO22_chloltmin_ss),#change at will - continuous vars only
             size = 1) +
  scale_colour_gradientn(limits = c(min(env_sub_meta$BO22_chloltmin_ss), max(env_sub_meta$BO22_chloltmin_ss)), #match variable acting as colour
                        colors=c('red','yellow','green'))+
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/SHELF_SOFT_1030__X_v_Y_chloltmin.png"), #amend
       device = "png",
       dpi=300 )
```
