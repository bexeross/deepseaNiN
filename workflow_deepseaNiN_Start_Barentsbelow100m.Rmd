...
```{r}
geodist_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_geodist_pa.rds"))
geodist_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_geodist_r6.rds"))

mds_pa<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_mds_pa.rds"))
mds_r6<-readRDS(file.path(dataPath,"inputs/SPLITS/DEEP/hiDens_mds_r6.rds"))
...
dim(otu_sort)
```
#### subsetting stations based on average ice cover
```{r}

joinedDat<- left_join(env_sort,otu_sort, by=c("SampID"="SampID"))

joinedDat_with_ice<-subset(joinedDat, BO22_icecovermean_ss >= 0.0001) # subsetting only stations below 100m depth with ice
joinedDat_no_ice<-subset(joinedDat, BO22_icecovermean_ss <= 0) # subsetting only stations below 100m depth with no ice
summary(joinedDat_no_ice$BO22_icecovermean_ss)
summary(joinedDat_with_ice$BO22_icecovermean_ss)

env_sort_no_ice<-joinedDat_no_ice %>% select(c(2:139)) # no ice
otu_sort_no_ice<-joinedDat_no_ice %>% select(c(2,140:528))

env_sort_with_ice<-joinedDat_with_ice %>% select(c(2:139)) # with ice
otu_sort_with_ice<-joinedDat_with_ice %>% select(c(2,140:528))


#table(env_sort_no_ice$SampID2 == otu_sort_no_ice$SampID)
dim(env_sort_with_ice)
dim(otu_sort_with_ice)
...
```{r}
saveRDS(geodist_pa,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/Barents_below100m_geodist_pa.rds")))
```
...
```{r}
saveRDS(mds_pa,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/Barents_below100m_mds_pa.rds"))
```
...
png(file=file.path(dataPath,"outputs/Barents_below100m_procrustes_pa.png"), width=1000, height=700)
plot(procr_pa)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/Barents_below100m_correlationPCAvsNMDS_PA.png"),
       device = "png",
       dpi=300 )
...
```{r}
saveRDS(geodist_r6,
        file = (file.path(dataPath,"inputs/SPLITS/BARENTS/Barents_below100m_geodist_r6.rds")))
```
...
```{r}
saveRDS(mds_r6,
        file = file.path(dataPath,"inputs/SPLITS/BARENTS/Barents_below100m_mds_r6.rds")) 
```
...
png(file.path(dataPath,"outputs/Barents_below100m_procrustes_r6.png"), width=1000, height=700,) #added 1000
plot(procr_r6)
dev.off()
```
...
##### Save the figure
```{r}
ggsave(filename = file.path(dataPath,"outputs/Barents_below100m_correlationPCAvsNMDS_r6.png"),
       device = "png",
       dpi=300 )
...
##### Save the figure
```{r}
ggsave(file.path(dataPath,"outputs/Barents_below100m_gnmds_dca_r6.png"),
       device = "png", 
       dpi=300)
```
...
write.csv(x = cor_tab,
          file = file.path(dataPath,"inputs/SPLITS/BARENTS/Barents_below100m_cor-table_r6_200rep.csv"),
          row.names = FALSE)
```
...
ggexport(ordsrfs_plt,
          filename = file.path(dataPath,"outputs/Barents_below100m_ordisurfs_top_corr.png"),
          width = 1000,
          height = 1500)
...
env_os_m <- env[,c("Tmean_Robinson", #top corr
                  ...
                  "BO22_chlomean_ss",# high corr with ax1
                  "BO22_chlorange_ss",# high corr with ax1
                  "BO22_ppltmax_bdmean", # corr with ax1
                  "X.y", #comparison to Y
                  "Y", #top corr
                  "spd_mean", 
...
ggexport(ordsrfs_plt_m,
          filename = file.path(dataPath,"outputs/Barents_below100m_ordisurfs_man_sel.png"),
          width = 1000,
          height = 1000)
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/Barents_below100m_EnvFit_man_sel.png"),
       device = "png",
       dpi=300 )
```
...
#### gnmds w ice cover
```{r}
ice <- ggplot(data = env_vis,
                     aes(x = gnmds1,
                         y = gnmds2)) +
  theme_classic() +
  coord_fixed() +
  ggtitle("GNMDS coloured by average ice cover",
          subtitle = "First run") +
...
c25 <- c(
  ...
  "#FDBF6F", # lt orange
  "gray70", "khaki2",
  "maroon", "orchid1", "pink", "blue1", "steelblue4",
...
```{r}
cat_var_plots<-ice+p_mld+p_sed+p_land+p_gmo
```
##### Save the plot
```{r}
ggexport(cat_var_plots,
          filename = file.path(dataPath,"outputs/Barents_below100m_gnmds_catvar.png"),
          width = 1000,
          height = 800)
```
