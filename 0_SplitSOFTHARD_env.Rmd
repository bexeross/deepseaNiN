...
vsoftCols<-c("vClay", "vClayOutcrop",   "vClay_spicules", "vMud", "vSandy_mud" )
vcoarseCols<-c("vSand",  "vSand_with_biomaterial", "vSand_with_shells", "vPebble")
vhardCols<-c("vBedrock", "vBoulder",  "vCobble", "vCompacted_sediment", "vKarbonatskorpe", "vDead_lophelia", "vCoral_rubble",  "vConsolidated_clay")
...
env_orig<-env_orig %>% 
  mutate (sedPropCatHI=if_else(vHard>vCoarse & vHard>vSoft, "Hard", if_else(vSoft>vCoarse & vSoft>vHard, "Soft","Coarse")))
...
env_orig<-env_orig %>% 
  mutate (sedclassCatNGU=if_else(sedclass>173, "Hard", if_else(sedclass<94, "Soft","Coarse")))
...
env_orig<-env_orig %>% 
  mutate (sedPropCatNGU=if_else(HardNGU>CoarseNGU & HardNGU>SoftNGU, "Hard", if_else(SoftNGU>CoarseNGU & SoftNGU>HardNGU, "Soft","Coarse")))
...
leftovers<-subset(env_orig, is.na(PredomHardSoft))
