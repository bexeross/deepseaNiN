...
#### X v Y - bathy
```{r}
xy_b<- ggplot(data = env_sub_meta,
                      aes(x = X,
                          y = Y)) +
  theme_classic() +
  geom_point(aes(colour = bathy),#change at will - continuous vars only
             size = 1) +
  #scale_colour_gradientn(limits = c(min(bathy), max(bathy)), #match variable acting as colour
  #                      colors=c('blue','green','red'))+
  scale_colour_gradientn(limits = c(-400, -100), #match variable acting as colour
                        colors=c('blue','green','red'))+
  ggtitle("Geography (X v Y) - coloured by mean bathy") #amend

#xy_t #plot
ggplotly(xy_b) #interactive plot

```
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_X_v_Y_bathy.png"), #amend
       device = "png",
       dpi=300 )
```
...
gnmds_spR <- ggplot(data = env_sub_meta,
  ...
  #                       colors=c('red','yellow','green'))+
  scale_colour_gradientn(limits = c(5,50),#match variable acting as colour
                        colors=c('red','yellow','green'))+
...
gnmds_gmo <- ggplot(data = env_sub_meta,
  ...
  ggtitle("GNMDS coloured by landscape class",
          subtitle = "First run") +
  geom_point(aes(colour = factor(landscape))) + #change at will - categorical vars only
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
gnmds_gmo <- ggplot(data = env_sub_meta,
  ...
  ggtitle("GNMDS coloured by biotope",
          subtitle = "First run") +
  geom_point(aes(colour = factor(BiotopeDesc))) + #change at will - categorical vars only
  geom_vline(xintercept = 0,
             linetype = 2,
             colour = "lightgray") +
  geom_hline(yintercept = 0,
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_gnmds_biotope.png"), #amend
       device = "png",
       dpi=300 )
...
##### Save the plot
```{r}
ggsave(filename = file.path(dataPath,"outputs/sandwave_shelf_XvY_ax1cat.png"), #amend
       device = "png",
       dpi=300 )
...
#### X v Y with gnmds ax 1 as HC categories
```{r}
cat_xy_sampID <- ggplot(data = env_sub_meta,
              aes(x = X,
                  y = Y)) +
  theme_classic() +
  geom_point(aes(colour = SampID), #change at will - categorical vars only
             size = 1) +
 # scale_colour_manual(values=cbPalette) +# non-ordered colourblind pallette
  #scale_colour_brewer(palette = "Spectral") + # ordered colourblind pallette
  ggtitle("Easting vs Northing - coloured by sampleID") #amend

#cat_xy_ax1 #plot
ggplotly(cat_xy_sampID) #interactive plot
```
